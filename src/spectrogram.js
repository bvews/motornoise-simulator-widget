...
Spectrogram.prototype.generateColorMap = function (colorStops) {
    const dark = [0, 0, 255];
    ...
    for (let i = 0; i < 256; i++) {
        ...
        this._colors[i] = color.map(function (value) { return Math.round(value); });
    }
};

Spectrogram.prototype.setFrequencyListText = function (text) {
    this._frequencies = text.split(/[\n, ]/)
        .map(function (frequency) { return parseFloat(frequency); })
        .filter(function (frequency) { return !isNaN(frequency); });
...
Spectrogram.prototype._drawFrequencyMarkers = function () {
    if(!this._analyserNode) {
        return;
    }
    ...
    this._frequencies.forEach(function (frequency) {
...
Spectrogram.prototype._clear = function (canvas) {
...
Spectrogram.prototype.setAnalyser = function (analyserNode) {
...
Spectrogram.prototype.setDecibelsRange = function (min, max) {
    if (this._analyserNode) {
...
Spectrogram.prototype.setFftSize = function (fftSize, isAdjust) {
    if (this._analyserNode) {
